...
class Simulation:
    ...
    def __init__(
        ...
        # ROCKSTAR 
            ...
            # Read in raw snapshot(s) to access star particle coordinates for scatterplots
            self.particles = gizmo.io.Read.read_snapshots(
                simulation_directory = simulation_directory,
                snapshot_directory=SNAPSHOT_DIRECTORY,
                species = species,
            ...
            before_filter_total = len(self.hal['mass.vir'])
            ...
            if ('star' in species) or ('all' in species): 
                ...
                for dm_halo in range(len(self.hal['mass.vir'])):
                    low_res_frac = self.hal['dark2.mass'][dm_halo] / self.hal['mass.vir'][dm_halo]
                ...
                # For each halo in [total number of halos]
                for dm_halo in range(len(self.hal['mass.vir'])):
                    low_res_frac = self.hal['mass.lowres'][dm_halo] / self.hal['mass.vir'][dm_halo]
                    # Calculate the complement of low_res_frac (the value that when added, will equal 1)
                    hi_res_frac = 1 - low_res_frac
                    # Filter out halos whose hi_res_frac value exceeds 0.99 
                    if hi_res_frac > 0.99:
            ...
            self.sorted_keys = sorted(range(len(self.hal['mass.vir'])), key=lambda x: self.hal['mass.vir'][x], reverse=True)

            # Get the total number of halos so we can check if a get_halo() search is valid
            self.totalHalos = len(self.hal['mass.vir'])
