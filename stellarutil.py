import os, gizmo_analysis as gizmo, astropy.io.ascii as ascii, numpy as np, halo_analysis as halo, h5py
...
class RockstarHalo:
    ...
    def restrict_percentage(self, percentage = 15):
        ...
        new_stars = []
        for i in range(len(x_gal)):
            star = RockstarStar(x_gal[i], y_gal[i], z_gal[i], m_gal[i], a_gal[i], vx_gal[i], vy_gal[i], vz_gal[i])
            new_stars.append(star)
        # Update the halos star list
        self.stars = new_stars
        ...
        if self.dark_particles:
            ...
            for i in range(len(x_dark_gal)):
                dark = RockstarDark(x_dark_gal[i], y_dark_gal[i], z_dark_gal[i], m_dark_gal[i])
...
class RockstarSimulation:
    def __init__(
        ...
        # Check if 'star' is included along with 'all' or any other species
        if 'star' in species and ('all' in species or len(species) > 1):
            # If star is included with any other species or if ['all'] is used, inform the user of missing information.
            print("The following information (all star information) is unavaible when a species (e.g. gas, dark, etc) is provided " +
                  "in addition to ['star'] OR if ['all'] is used.")
            print(" - Dark2.mass")
            print(" - star.form.time.100")
            print(" - star.form.time.50")
            print(" - star.form.time.90")
            print(" - star.form.time.95")
            print(" - Star.form.time.dif.68")
            print(" - Star.indices")
            print(" - star.mass")
            print(" - Star.massfraction")
            print(" - Star.number")
            print(" - Star.position")
            print(" - Star.radius.50")
            print(" - Star.radius.90")
            print(" - Star.vel.std")
            print(" - star.vel.std.50")
            print(" - star.velocity")
        ...
        print(f"Total halos filtered: {before_filter_total - after_filter_total}")
